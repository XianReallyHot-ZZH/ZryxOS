...
- **ZryxOS 跟 OpenClaw、Hermes 的关系**是同类不同定位。三者都是 Agent OS，OpenClaw 偏个人、Hermes 偏个人到小团队，ZryxOS 直接定位严监管企业场景。三者都采用 markdown + frontmatter 的目录形态，社区的优质 Skill 经过企业审查后理论上可以导入 ZryxOS 的全局 Skill 库（`.oryxos/skills/`）。
...
3. **Skill**：可复用的 Agent 能力模板，用 `SKILL.md` 文件格式描述，兼容 agentskills.io 开放标准。OpenClaw 和 Hermes 都用这个格式。在 ZryxOS 里 Skill 是**全局共享能力库**（`.oryxos/skills/<name>/SKILL.md`），Agent 按名引用、正文注入 system prompt；Agent 本身的定义本体则是 `.oryxos/agents/<name>/AGENT.md`。
