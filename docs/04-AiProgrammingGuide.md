...
| 阶段 | 工具 | 适用场景 |
|------|------|---------|
| **主体开发阶段** | Spec-Kit | 从零开发 ZryxOS 1.0 的五大核心能力，9 个 Maven 模块 |
...
写实施计划之前先回答一个根本问题：Spec-Kit 真的适合 ZryxOS 项目吗？
...
| AI agent 协作 | ZryxOS 本来就是用 Claude Code 做主体开发 | ✅ 完全匹配 |
...
| 局限 | 描述 | ZryxOS 的应对 |
|------|------|-------------|
| 流程对小增量过重 | Spec-Kit 完整流程对小改动开销过大 | ZryxOS 主体开发是大颗粒度，增量阶段切换到手动提示词 |
| spec 不会自动跟实现同步 | AI agent 在 implement 阶段可能偏离 spec | 每个 user story 实施完成后跑 `/speckit.analyze` 做一致性检查 |
| context limit 在大型 brownfield 上失效 | 十万级文件的 legacy 项目 LLM 看不全 | ZryxOS 是纯 greenfield，整个项目在 LLM context window 内，不适用 |
...
Specify CLI 是 Spec-Kit 的入口工具（Python 实现，需要 Python 3.11+，推荐用 `uv` 安装）。安装后通过 `specify init` 初始化 ZryxOS 项目的 Spec-Kit 工作区，工作区里有 `.specify/memory/constitution.md` 以及 spec、plan、tasks 等 artifacts 的目录结构。
...
`constitution.md` 是项目的 **non-negotiable principles**，所有后续 spec、plan、tasks、implement 都要遵守。ZryxOS 的 constitution 从需求文档第 3 章设计目标 + 技术方案第 1.1 节关键技术决策提炼：
...
5 个 user story 按依赖关系排推进顺序，而不是按重要性。这里要特别说明：US-5 Web Service 排在最后实施，是因为它依赖前四个能力都就绪，**不是因为它不重要**。恰恰相反，Web Service 是 ZryxOS 区别于个人助手项目的关键能力，重要性很高。本文档不用 P1/P2/P3 这种优先级标记，避免被误读成"靠后的可以不做"，只讲依赖顺序。
...
- `zryxos-tool`（一个 HTTP Tool + `SandboxChecker` 简化版，Demo 一需要）
- `zryxos-channel-cli`（CLI Channel，Demo 一需要）
- `zryxos-cli`（`zryxos init` + `zryxos chat` 命令）
...
`zryxos chat` 启动 CLI，用户输入"查一下北京天气并告诉我穿什么"，Agent 通过 ReAct 循环调用 HTTP Tool 拉天气 JSON，根据数据回复穿衣建议，完整对话日志正确累积到 Session，至少跑通一个 Provider（DeepSeek 或 Kimi）。
...
**涉及的 Maven 模块**：
- `zryxos-memory`（核心能力三，含 `MemoryService` 三层门面、`LongTermMemory`、`MemoryTools`）
...
第一次对话告诉 Agent"我项目用 Spring Boot，部署在 K8s 上"，Agent 主动调 `save_memory` 追加到 `MEMORY.md`；重启 ZryxOS 或新开会话；第二次对话问"帮我看看我的项目能用什么数据库"，Agent 在响应里引用之前记的偏好给出建议。
...
**涉及的 Maven 模块**：
- `zryxos-tool`（补齐文件 Tool + Shell Tool、MCP Client、`SandboxChecker` 完整版、`ToolRegistry`，三合一模块）
...
| `AGENT.md` 类 | `ContextLoader` 加载 `.zryxos/agents/` 下每个 Agent 的 `AGENT.md` 正文拼接到 system prompt，这部分归 core 不归 tool |
...
业务方写一个 Agent 目录 `.zryxos/agents/daily-pr-digest/`（`AGENT.md` 正文描述任务），在 `mcp_servers.yaml` 配置 `github-mcp`（用社区现成的 MCP server），Agent 启动后能读 `AGENT.md` 正文、调 `github-mcp` 拉 PR、汇总成简报，整个过程业务方零代码只写了一个目录 + 配置。
...
看到 Claude Code 生成的代码不符合 constitution，主动让 AI agent 重读 constitution 改正。ZryxOS 最容易被写错的几个点：
...
| 把 Tool 又拆成多模块 | 应该合并为一个 `zryxos-tool` 模块 |
...
ZryxOS 作为开源项目需要一个独立的主页作为对外门面，技术栈用 VitePress 或类似静态站点工具，内容讲清楚 ZryxOS 是什么、五大核心能力是什么、怎么快速开始。
...
**Spec-Kit 跟 ZryxOS 的契合度很高**：纯 greenfield、medium 规模（9 个模块）、需求清晰、AI agent 协作、方法论场景，每条都对得上。

**核心策略是已有文档喂给 Spec-Kit，不重写**。ZryxOS 已经投入了完整的业界调研 + 需求文档 + 技术方案，这些是 Spec-Kit 的最佳输入。**关键是喂的是最新版文档**：模块是 9 个不是 11 个，constitution 要包含"Spring AI 只用一半"、"审计 day one 落库"这些新决策，否则 Spec-Kit 生成的 plan 会按旧结构走偏。
