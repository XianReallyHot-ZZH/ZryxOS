...
业界已经有开源 Agent 项目把这套设计验证过（OpenClaw 用 Node.js，Hermes Agent 用 Python），但 Java 生态没有任何项目把"Agent OS"作为定位。Java 是大量企业现有后端的事实标准技术栈，Spring AI Alibaba 已经把底层 LLM 调用解决了，缺的就是上面那一层"Agent OS"。ZryxOS 填这个位置。
...
理解这个分层，才能看懂 ZryxOS 的交付节奏：

1. **核心阶段**：先把 Agent OS 的运行时内核用 Java 做扎实，这一层在能力上对齐业界开源 Agent OS 的基础层
2. **扩展阶段**：ZryxOS 真正的差异化治理层（多租户、SSO、完整审计、Tool 治理），在核心内核之上由扩展阶段和社区共建陆续补齐
...
ZryxOS 优先做五个核心能力，基于这五个能力可以扩展出企业里大量真实需求。这五个能力都属于"让单个 Agent 跑得好"的运行时内核层；让 ZryxOS 成为真正"OS"的多 Agent 治理能力（多租户、Tool Policy、审计、SSO），在扩展和社区阶段补齐。
...
| 术语 | 定义 |
|------|------|
| **Agent（智能体）** | 一个具象的业务智能体，定义本体是一个目录 `.zryxos/agents/<name>/`：`AGENT.md`（frontmatter 是这个 Agent 自己的 profile——用哪个模型、能用哪些工具、绑定哪个渠道、要不要定时；正文是任务指令——做什么、什么时候该做）加可选 `skills/*.md` 子指令、`scripts/` 脚本、`REFERENCE.md` 参考。一个目录就是一个完整可用的业务 Agent，不是写代码写出来的 |
...
| **Web Service** | ZryxOS 对外暴露的完整 REST API，是业务系统集成 ZryxOS 的唯一通道 |
...
| **Skill（技能）** | 全局共享的能力库，存 `.zryxos/skills/<name>/SKILL.md`（frontmatter 加约束指令正文）。Agent 在 `AGENT.md` frontmatter 用 `skills: [名]` 按名引用，`ContextLoader` 组装 system prompt 时把引用到的 Skill 正文注入，用来强约束产出。Skill 不是可执行 Tool，不进 `ToolRegistry` |
| **Bootstrap（引导文件）** | 加载到系统提示词中的上下文文件：AGENTS.md（项目级 agent 行为说明）、SOUL.md（agent 人格定义）、USER.md（用户偏好） |
| **Workspace（工作区）** | ZryxOS 实例的工作目录，默认是 `.zryxos/`，包含 Agent 目录、全局 Skill 库、Bootstrap 文件、记忆、会话、产出物、日志的子目录 |
...
以下三个典型场景描述 ZryxOS 完整形态（含扩展阶段能力）下的目标用法，核心阶段先具备其运行时内核。

### 场景一：运维助手

某中型 SaaS 公司的运维团队基于 ZryxOS 搭一个运维助手，接入企业微信。Agent 配了几个 Tool（告警分诊、日志查询、服务重启、变更审批）。凌晨告警通过 webhook 进 ZryxOS，Agent 收到告警后调用日志查询 Tool 拉错误堆栈，跟历史故障库交叉引用发现是已知 bug，自动应用 mitigation Skill 重启服务，在企业微信运维群里汇报"已自愈，详情见附件"，值班工程师早晨起来看下记录就行。

**ZryxOS 在此场景的角色**：Channel 接入（企业微信）、Provider 路由（主备 LLM）、Tool 调用（SSH、Prometheus、Slack 通知）、Memory（历史故障库）、Skill（自愈 runbook）。

### 场景二：知识管理助手

某金融企业的法务团队基于 ZryxOS 搭一个知识管理 Agent，接入飞书。Agent 索引了内部的合同模板、法规文档、历史案例、咨询记录。员工在飞书里问"上次签 SaaS 服务协议是怎么处理数据出境条款的"，Agent 检索 Memory 拉出历史案例，综合相关法规给出建议草稿，标注引用来源。

**关键点**：Memory 检索准确度和引用追溯（合规要求所有 Agent 回复必须可追溯到引用源）。

### 场景三：销售助手

某制造业企业的销售部门基于 ZryxOS 搭一个客户洞察 Agent，接入企业微信和 CRM。销售跑客户前问 Agent"明天去拜访 A 公司，有什么我需要知道的"，Agent 调用 CRM connector 拉客户历史交易记录，调用企查查 MCP 工具查最新工商信息，调用知识库 Tool 提取关键决策人和采购习惯，综合输出客户简报。

**ZryxOS 在此场景的核心能力**：MCP 集成（外部数据）、企业 IT 系统 connector（自家 CRM）、Tool 编排。
...
- `zryxos init` 幂等：已存在的目录和文件一律不覆盖。六个子目录建好后，用 `zryxos profile create <name>` 生成第一个 Agent 目录

---

### 5.2 定义一个 Agent：AGENT.md

一个目录 = 一个 Agent。`.zryxos/agents/<name>/AGENT.md` 由两部分组成：**frontmatter** 是这个 Agent 自己的 profile（用哪个 Provider/模型、能用哪些 Tool、引用哪些全局 Skill、绑定哪个 Channel、要不要定时），**正文**是任务指令（"这个 Agent 具体做什么"）。`AgentLoader.deriveProfile()` 把 frontmatter 派生成底座认识的 `Profile`，所以不需要另写一份 Profile YAML。
...
skills:
  - string                      # 按名引用全局 Skill 库（.zryxos/skills/<名>/），正文注入 system prompt
...
**Agent 管理命令**（命令组名沿用 `profile`，操作的是 `.zryxos/agents/` 下的 Agent 目录）：

```bash
zryxos profile create <name>    # 创建 Agent（生成最小 AGENT.md 模板）
zryxos profile list             # 列出全部 Agent
zryxos profile show <name>      # 查看某个 Agent 的 AGENT.md
zryxos profile delete <name>    # 删除 Agent（整个目录）
```
...
**用户核心体验**：用 ZryxOS 一段时间后，Agent 自然会记住用户的偏好、项目信息、关键决策，下一次对话不需要重新解释。这是 Agent OS 区别于 chatbot 的核心体验。

---

### 5.6 Tool 体系（核心能力四：让 Agent 能干事）

Tool 是 Agent 可以调用的外部能力。Agent 通过 LLM Function Calling 决定何时调哪个 Tool，ZryxOS 负责 Tool 的注册、查找、调用、结果回传。
...
1. 建 `.zryxos/agents/daily-pr-digest/`，写一份 `AGENT.md`：frontmatter 声明 provider、`mcp_servers`、`schedules`，正文写任务指令
...
Web Service 是 ZryxOS 的对外完整门面，业务系统通过 REST API 接入 ZryxOS 的所有能力。这是 ZryxOS 区别于偏个人定位的 OpenClaw、Hermes 的关键能力。
...
主页在核心阶段做出来，与核心代码同期发布，作为 ZryxOS 1.0 对外亮相的一部分。技术栈推荐使用 VitePress、Astro 或 Docusaurus 等静态站点生成器。
...
- **Skill 库增强**：全局 Skill 库（`.zryxos/skills/<name>/SKILL.md`，按名引用、正文注入）在核心阶段已落地；扩展阶段补语义检索选 Skill、版本管理、以及从 agentskills.io 等开放格式导入时的企业审查流程

### 6.3 工具和安全层

- **MCP Server 暴露**：ZryxOS 自己作为 MCP server，把内部 Agent 能力暴露给其他系统
...
社区共建功能不在 ZryxOS 主线开发计划内，作为长期方向开放给社区贡献，不规定时间表。
...
- **多区域部署**：跨地域 ZryxOS 集群，Agent、Memory、Session 可以跨区域协同
...
（LLM 调用本身的延迟取决于 Provider，不在 ZryxOS 控制范围内）
...
- **LLM 协议**：OpenAI 兼容协议是事实标准，只要 Provider 实现这套协议，ZryxOS 就能直接接
...
- **数据驻留**：ZryxOS 不主动外发任何数据，所有数据留在企业自己的基础设施上
...
用户执行 zryxos profile create <name>
  → ZryxOS 在 .zryxos/agents/<name>/ 下创建 AGENT.md（最小模板）
...
消息从 Channel 进来（CLI 输入或 HTTP API 调用）
  ...
  → 调用 LLM Provider 获取响应
  → [有 Tool 调用] → ZryxOS 执行 Tool → 结果回传给 LLM 继续生成
...
LLM 通过 Function Calling 指明 Tool 名称和参数
  → ZryxOS 从 Agent 工具池找到对应 Tool
  → 做参数校验和白名单校验
  → 内置 Tool：在 ZryxOS 进程内执行（白名单约束下）
...
```
用户第一次跟 Agent 说话
  → ZryxOS 用 Channel+用户+Agent 联合 ID 查活跃 Session
...
ZryxOS 核心功能的实施按 **4 周节奏**组织，每周 3 小时，合计 12 小时。
...
- `zryxos init` 工作区初始化、`AGENT.md` frontmatter 解析
...
| **Tool 执行安全风险** | 应用层白名单不是完整 Sandbox，可能影响 ZryxOS 进程 | 严格限制内置 Tool 能力范围，核心阶段不建议在生产环境跑高敏感场景 |
...
| 事项 | 说明 | 决议时间 |
|------|------|---------|
| Provider 抽象接口设计 | 直接用 Spring AI `ChatClient`，还是在 `ChatClient` 之上加一层 ZryxOS 自己的抽象 | 技术方案阶段 |
...
2. ZryxOS 真正的差异化治理层（多租户、SSO、完整审计、Tool 治理），在核心内核之上由扩展阶段和社区共建陆续补齐。**核心阶段是地基，企业级治理是终局。**
